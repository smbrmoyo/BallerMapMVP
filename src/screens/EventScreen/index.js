...
import { onCreateUserEventConnection, onDeleteUserEventConnection } from '../../graphql/subscriptions';
  ...
  const subscribeToCreateEvent = async (profileDocument, loggedUser) => {
    ...
    await API.graphql({
      ...
      next: (subOnDeleteConnection) => {
        console.log(subOnDeleteConnection.value.data.onCreateUserEventConnection);
        const profileId = profileDocument !== null? profileDocument.id: JSON.parse(loggedUser).email;
        if(subOnDeleteConnection.value.data.onCreateUserEventConnection.profileID == profileId){
          console.log('user related to this event');
          let newEvents = [];
          newEvents.push(myEvents);
          //Add this event
          newEvents.push(subOnDeleteConnection.value.data.onCreateUserEventConnection.Event);
    ...
    await API.graphql({
      ...
      next: (subOnDeleteConnection) => {
        ...
        if(subOnDeleteConnection.value.data.onCreateUserEventConnection.profileID == profileId){
          ...
          newEvents.push(myEvents);
          //Remove this event
          newEvents.splice(myEvents.indexOf(subOnDeleteConnection.value.data.onCreateUserEventConnection.eventID, 1));
          setMyEvents(newEvents);
        } else {
          console.log('event not related to this user');
        }
